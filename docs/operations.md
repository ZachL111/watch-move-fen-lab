...
`watch-move-fen-lab` is centered on chess and game engines work in Elixir. The technical target is: Build an Elixir toolkit that studies fen behavior through negative fixtures, with human-readable error snapshots and no network dependency.
...
The score is deterministic. The current threshold is 163. Risk has a penalty of 7, latency has a penalty of 2, and weight contributes a bonus of 4.
